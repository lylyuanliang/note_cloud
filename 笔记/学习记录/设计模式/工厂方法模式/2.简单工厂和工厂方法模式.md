**简单工厂**和**工厂方法模式**都用于将对象的创建过程与使用过程分离，以提高代码的灵活性和可维护性。需要注意的是，简单工厂是一种常见的创建对象方式，但不属于 GoF 23 种设计模式；工厂方法模式才是 GoF 创建型设计模式之一。

我们以下面场景为例：定义一个日志系统，有多种日志输出方式（如文件日志、数据库日志）。我们可以使用简单工厂或工厂方法模式来实现日志对象的创建。

### 1. 简单工厂模式

简单工厂模式（Simple Factory）通过一个**工厂类**来创建产品对象，它根据传入的参数决定实例化哪种具体产品。简单工厂常用静态方法实现，因此有时也叫静态工厂；但不要和 Java 通用的 `static factory method` 概念混淆。
...
public class Client {
    public static void main(String[] args) {
        Log log = SimpleLogFactory.createLog("file");
        log.write("系统启动");
    }
}
```
