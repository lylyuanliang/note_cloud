...
| bcdedit /enum\| find "hypervisorlaunchtype" | 查看 `hypervisorlaunchtype`状态<br />Auto: 这表示Hyper-V已启用，并且将在需要时自动启动。<br />Off: 这表示Hyper-V已禁用，并不会自动启动。<br />On: 这表示Hyper-V已启用，并且会在系统启动时自动启动。 |
